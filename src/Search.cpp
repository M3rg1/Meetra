...
namespace Search {
    ...
    static void InitSearchTimer(const Board &board) {
        if (!IsSearchLimited()) {
    ...
    static void InitNewSearch(const Settings &s, const Board &board) {
        start_time = Now();
        last_update_time = 0;
        mt_depth = 0;
        settings = s;
    ...
    static std::vector<RootMove> GenRootMoves(const Board &board) {
        MoveGen move_gen(board);
        std::vector<RootMove> root_moves;
        while (Move move = move_gen.NextMove()) {
            if (board.IsMoveLegal(move)) {
                root_moves.emplace_back(move, board.MoveEval(move));
            }
        }
    ...
    static Move GetBookMove(Hash64 hash) {
        if (auto moves = Book::Probe(hash); !moves.empty()) {
    ...
    void StartSearch(const Settings& settings, const Board &board) {
        ...
        if (use_book
            ...
            if (auto book_move = GetBookMove(board.Hash()); book_move != ZERO_MOVE) {
